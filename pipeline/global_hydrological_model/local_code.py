...
"""THIS CODE IS WAITING TO BE RUN""" """TRAINING FOR ONE COORDINATE AT A TIME"""


"""ABANDON RUNNING ON THE CLOUD FOR NOW, SINCE YOU'RE RUNNING INTO TOO MANY PROBLEMS AND YOU ARE GOING TO TRAIN YOUR MODEL ON YOUR GPU ANYWAYS"""
...
glofas = xr.open_mfdataset("/Volumes/portableHardDisk/data/2005/CEMS_ECMWF_dis24_*_glofas_v2.1.nc", combine="by_coords")
...
from functions.floodmodel_utils import get_river_mask
river_mask = get_river_mask(glofas['dis24'])
glofas_rivers = glofas.where(river_mask, drop=True)
...
inputs = dataset_total[len(dataset_total)-len(dataset_valid)-days_intake_length:]
...
inputs = dataset_total_2[len(dataset_total_2)-len(dataset_test)-days_intake_length:]
inputs = inputs.reshape(-1,1)
inputs = sc.transform(inputs)
y_test = []
X_test = []
...
for i in range(days_intake_length, len(inputs)-forecast_day+1):
    X_test.append(inputs[i-days_intake_length:i, 0])
...
plt.title('LSTM model 30-day forecasts with 90-day timesteps')
...
#Plotting the forecast of the 30th day of the test set
dataset_test.plot(label="True discharge", figsize=(15,5))
#Plotting the test predicated values
...
plt.title('LSTM model 30th-day forecasts with 90-day timesteps')
plt.legend(loc='upper left')
